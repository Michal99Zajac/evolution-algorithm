...
class X2SubjectAdapter(X2Subject, Valuer):
    def __init__(
        self,
        chromosomes: List[BinaryChromosome],
        length: int,
        left_limit: float,
        right_limit: float,
        ...
        indicators = [
            self.__transform(chromosome, left_limit, right_limit)
    ...
    def __transform(
        self, chromosome: BinaryChromosome, left_limit: float, right_limit: float
    ):
        return left_limit + int(str(chromosome), 2) * (right_limit - left_limit) / (
            2 ** len(self) - 1
        )
